  ...
  picSlaveData(0xA1)
{
  ...
  {
    handlers[i] = 0;
    SetInterruptDescriptorTableEntry(i, CodeSegment, &IgnoreInterruptRequest, 0, IDT_INTERRUPT_GATE); //0 is kernel space
  }
  ...
  // this tells os to add 20 and 28 whenever an interrupt comes
  picMasterData.Write(0x20);
  picSlaveData.Write(0x28);

  // init to tell which is master and slave
...
void InterruptManager::Activate()
{
  ...
  asm("sti"); // this is in diff method because in kernel we first init hardware then idt
}
